# -*- coding: utf-8 -*-
"""
Module name: SG2_main
Author:      Ryan Wu
Date:        2020/06/02
Description: Style GAN2 for FFHQ generation 
Training time: 11.3 seconds / epoch (@ RTX2080 ti GPU )
Training set:  FHHQ 70000 images
Versions:
...
Implementation detail:
    Loss            = Non-saturation loss 
    D regularization= R1 regularization
    G regularization= None
    D network       = Resnet (Conv + AveragePooling2D)
    G network       = Skip   (Upsampling2D + Conv)
    Normalization   = None (no Batch, layer, or group normalization)
    Optimizer       = Adam (beta1= 0; beta2= 0.99)
    Learning rate   = 2e-4 (D/G = 2)
    Image database  = FFHQ (70000 images with 512*512 size)
    Batch size      = 8
    
    
Image quality improve techniques    
    PG_GAN:     (X) Progressive growing
                (X) Equalized learning rate
                (X) Pixel wise normalization
                (O) Minibatch standard deviation
    Style_GAN:  (X) Adaptive instance normalization
                (X) Style mixing training
                (O) Stochastic variation (block B)
                (O) Disentangled mapping network 
                (O) D latent average
                (O) Truncation trick
    Style_GAN2: (O) Style modulation
                (O) Style normalization
                (O) Bilinear upsamling
                (X) Fused mod-demodulation
                (X) Lazy regularization
                (X) Path length regularization
...
class GAN():
    ...
    def train(self,
              ...
              model_name = 'SG2',                   # File name for model and snapshot
              #path       = '../../../Repository/FFHQ/00000/*.png',
              path       = '../../../Repository/FFHQ/*.png',
              **_kwargs):
        ...
        self.D_EMA       = SG2_utils.EMA(EMA_rate)   # EMA        
        self.GS_EMA      = SG2_utils.EMA(EMA_rate)   # EMA
        self.GM_EMA      = SG2_utils.EMA(EMA_rate)   # EMA
        ...
        self.GS          = SG2_model.G_Synthesis(dlatent_size= dlatent_dim)
        self.GM          = SG2_model.G_mapping(  latent_size= latent_dim, dlatent_size= dlatent_dim)
        self.D           = SG2_model.D_Style_GAN2()
    ...
    def _train(self):  
        ...
        ds = SG2_utils.load_dataset_FFHQ(self.path, self.buffer_size, self.batch_size)    
        ...
        while self.epoc < (self.epochs-5):
            for r_img in ds: 
                ...
                if np.round(self.epoc + siz/1000) > np.round(self.epoc):
                    ...
                    if (F+1)% 50  == 0: self.save_weights(fn)
                    if (F+1)% 20  == 0: self.save_weights()
    ...
    def load_weights(self):
        ...
        if os.path.isfile(fn + '_MDL_A.npy'):        
            ...
            self.G_history = B[2]
            self.D_history = B[3]        
            self.D_EMA.set_weights(B[4]); self.D.set_weights(B[4])
        else:
