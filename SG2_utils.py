...
Author:      Ryan Wu
Date:        V0.1- 2020/06/01: Initial release
...
#---------------------------------------------------- load images (FFHQ)
def load_png(path):
    png   = tf.io.read_file(path);                  # Load file
    img   = tf.image.decode_png(png);               # Decode as png
    return img 
    #----------------------------------------
def load_image(path):
    img   = load_png(path);
    ...
    return img
    #----------------------------------------
def load_image_FFHQ(path, pad =24):
    png   = tf.io.read_file(path)                   # Load file
    img   = tf.image.decode_png(png)                # Decode as .png
    img   = tf.pad(img,[[pad,pad],[pad,pad],[0,0]],'REFLECT')
    img   = tf.image.random_crop(img, [1024,1024,3])  # Random crop as square
    img   = tf.image.resize(img,(512,512))          # Resize as 512 x 512
    img   = tf.cast(img[:, :, :], tf.float32)       # Convert to tensor
    img   = img/127.5 - 1;                          # Normalize to [-1 ~ +1]    
    return img
    #---------------------------------------- 
def load_dataset_FFHQ(path, buffer_size, batch_size):
    ds    = tf.data.Dataset.list_files(path) 
    ds    = ds.map(load_image_FFHQ)
